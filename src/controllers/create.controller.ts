...
import { gigCreateSchema } from '@gigs/schemes/gig.scheme';
import { gigsServiceController } from '@gigs/services/gigs.service';
import { BadRequestError, ISellerGig, uploads } from '@jobhunt-microservices/jobhunt-shared';
import { UploadApiResponse } from 'cloudinary';
import { Request, Response } from 'express';
import { StatusCodes } from 'http-status-codes';

class GigsCreateController {
  create = async (req: Request, res: Response): Promise<void> => {
    ...
    const createdGig: ISellerGig = await gigsServiceController.createGig(gig);
    res.status(StatusCodes.CREATED).json({ message: 'Gig created successfully.', gig: createdGig });
  };
}

export const gigsCreateController = new GigsCreateController();
