...
import { elasticSearch } from '@gigs/elasticsearch';
...
class GigsServiceController {
  getGigById = async (gigId: string): Promise<ISellerGig> => {
    const gig: ISellerGig = await elasticSearch.getIndexedData('gigs', gigId);
    return gig;
  };

  getSellerGigs = async (sellerId: string): Promise<ISellerGig[]> => {
    const resultsHits: ISellerGig[] = [];
    const gigs = await searchService.gigsSearchBySellerId(sellerId, true);
  ...
  getSellerPausedGigs = async (sellerId: string): Promise<ISellerGig[]> => {
    const resultsHits: ISellerGig[] = [];
    const gigs = await searchService.gigsSearchBySellerId(sellerId, false);
    for (const item of gigs.hits) {
      resultsHits.push(item._source as ISellerGig);
    }
    return resultsHits;
  ...
  createGig = async (gig: ISellerGig): Promise<ISellerGig> => {
    ...
    if (createdGig) {
      ...
      await gigsProducer.publishDirectMessage(
        gigsChannel,
        ExchangeNames.SELL_CREATED,
        RoutingKeys.SELLER_CREATED,
  ...
  deleteGig = async (gigId: string, sellerId: string): Promise<void> => {
  ...
  updateGig = async (gigId: string, gigData: ISellerGig): Promise<ISellerGig> => {
  ...
  updateActiveGigProp = async (gigId: string, gigActive: boolean): Promise<ISellerGig> => {
    const document: ISellerGig = (await GigModel.findOneAndUpdate(
      { _id: gigId },
      {
        $set: {
  ...
  updateGigReview = async (data: IReviewMessageDetails): Promise<void> => {
  ...
  seedData = async (sellers: ISellerDocument[], count: string): Promise<void> => {
...
export const gigsServiceController = new GigsServiceController();
