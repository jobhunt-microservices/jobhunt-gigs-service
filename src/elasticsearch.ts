...
const log = logger('gigsElasticSearchConnection', 'debug');
...
class ElasticSearch {
  ...
  addDataToIndex = async (index: string, itemId: string, gigDocument: unknown): Promise<void> => {
    try {
      ...
      log.log('error', 'GigService elasticsearch addDataToIndex() method error:', getErrorMessage(error));
    }
  };
  ...
  updateIndexedData = async (index: string, itemId: string, gigDocument: unknown): Promise<void> => {
    try {
      ...
      log.log('error', 'GigService elasticsearch updateIndexedData() method error:', getErrorMessage(error));
    }
  };
  ...
  deleteIndexedData = async (index: string, itemId: string): Promise<void> => {
    try {
      ...
      log.log('error', 'GigService elasticsearch deleteIndexedData() method error:', getErrorMessage(error));
    }
  };
}
